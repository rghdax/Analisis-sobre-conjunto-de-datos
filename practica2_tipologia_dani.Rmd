...
df2 <- df[df$Release.Year>2004 & !is.na(df$Runtime),]
...
attach(df2)
model.log=glm(formula=Fresh.log~Runtime+factor(Parental.Control)+factor(Release.Year)+factor(DirectorIsWriter)+factor(Release.isWide),family=binomial(link=logit))
summary(model.log)
...
```{r regresion lineal}
model.lin=lm(Tomatometer~Runtime+factor(Parental.Control)+factor(Release.Year)+factor(DirectorIsWriter)+factor(Release.isWide),data=df2)
...
qqnorm(residuos, main=bquote(~ residuos == .(length(residuos))))
qqline(residuos)
