...
Se utilizarán las variables `Tomatometer` y `DirectorisWriter`.
   ...
   - ¿Las películas del género drama tienen una mejor puntuación por parte la audiencia que las pelñiculas de terror?
...
*Las conclusiones a cada gráfico se han comentado en el apartado 4.3. En este apartado, se muetra un resumen de esas conclusiones.*
  ...
  - Las variables Genre2 y Genre3 se han descartado por tener demasiados valores perdidos. Director y Production no se utilizarán por tener una cantidad de categorías demasiado grande. Rating y Release.Date no se usarán porque en su lugar se usarán Parental.Control y Release.Year. Seasons no se utilizará por no tener ningñun efecto apreciable en las puntuaciones de las películas.
  
  - Además de las etiquetas Tomatometer, Audience.score y Fresh, las variables que se usarán en los modelos de clasificación o regresión son: **`Parental.Control`, `DirectorIsWriter`, `Runtime`, `Genre1`, `Release.Year` y `Release.isWide`**.
