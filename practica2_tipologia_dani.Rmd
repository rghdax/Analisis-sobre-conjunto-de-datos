...
Se realizará un análisis de regresión para predecir la `Tomatometer` de una película. Para ello se escogerán, a partir del análisis de correlación de las variables, cuáles son las variables de entrada que se utilizarán entre `Runtime`, `Parental.Control`, `Release.Year`, `Genre1`, `Release.isWide` y `DirectorIsWriter`.
...
Se utilizarán las variables `Tomatometer` y `DirectorisWriter`.
   ...
   - ¿En otoño se estrenan más películas de horror que en las demás estaciones?
...
p45 <- ggplot(data = df,aes(x=Rating,fill=Fresh))+geom_bar() + 
...
p49 <- ggplot(data = df,aes(x=Release.Season,fill=Fresh))+geom_bar() + 
       ggtitle("Fresh by release Release.Season")

p410 <- ggplot(aes(y = Audience.score, x = Release.Season), data = df) + 
        geom_boxplot() + ggtitle("Audience score by release Release.Season")

p411 <- ggplot(aes(y = Tomatometer, x = Release.Season), data = df) + geom_boxplot() + 
        ggtitle("Tomatometer by release Release.Season")
...
df2 <- df[c("Tomatometer","Fresh", "Parental.Control","DirectorIsWriter",
...
Se crea el modelo de regresión logística, donde la variable dependiente es `Fresh` y las independientes `Parental.Control`, `DirectorIsWriter`, `Runtime`, `Release.Year` y `Release.isWide`, obteniendo un AIC de 8250,1.
...
```{r regresion lineal}
model.lin=lm(Tomatometer~Runtime + factor(Parental.Control) + 
...
#### Contraste 1: Las películas dirigidas por el propio escritor son mejores (tienen mayor Tomatometer) que las que tienen un director distinto

Lo primero será separar las dos muestras, por un lado las películas cuyo director es también el escrito y las que no y las graficamos para observar su distribución:
```{r Separación de muestras}
...
A partir de lo anterior, se aplicará el análisis de contraste sobre la diferencia de medias cuando la **distribución de las varibales es normal y la varianza de ambas es desconocida pero igual**.
